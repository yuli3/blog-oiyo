...
- 학문형 academy의 중급 복습 기준(대학 이수자의 취업·직무·시험 복습)은 `npm run audit:intermediate-academy:check`다. 설정은 `config/intermediate-academy.json`. 암기 카드·시험 템플릿 출처·도출·수치 예·한계·확인문제가 없는 페이지를 `enforcedSeries`에서 실패로 막는다. 글자 수로는 통과시키지 않는다.
