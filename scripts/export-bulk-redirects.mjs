...
// Pages-only rules emitted before everything else. Cloudflare Pages applies
// _redirects even when a static asset exists, so the root shell (index.html)
// becomes a single server 301 to the canonical locale home (2026-09-24).
// Kept out of data/redirects/canonical-redirects.txt so it is never exported
// to Bulk Redirects and never lost on regeneration.
const PAGES_FIRST_RULES = [
  { raw: "/  /en/  301", reason: "root to canonical locale home (Pages only)" },
];
