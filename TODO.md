# Oiyo Content Strategy & Future Topics

This document outlines the core values, writing guidelines, and planned topics for the Oiyo blog. Every article must adhere to these standards to ensure a "New York Times / FAANG-level" reading experience.

## Core Writing Principles

1.  **Level of Quality**: Every article should feel like a deep-dive essay or a research paper, yet remain accessible.
2.  **Volume & Structure**:
    - Minimum **3000 characters** per article.
    - Minimum **5 major paragraphs/sections**.
    - Use clear headings (`##`, `###`) to structure the content logic.
3.  **Tone & Voice**:
    - **Kind and Gentle**: Use an empathetic and warm tone.
    - **Simple Explanations**: Capable of explaining complex psychological or scientific concepts in a way that anyone can understand.
    - **Insightful**: Avoid hollow advice; provide "Aha!" moments through deep reasoning.
4.  **Workflow**:
    - Write the **Korean (ko)** version first as the master copy.
    - Translate and localize into **English (en)** and **Japanese (ja)**.
    - **Cross-Linking**: Link to relevant Wikipedia pages and internal Oiyo resources (Psychological counseling, MBTI, TAI, HSP, etc.) using `target="_blank"`.

## Multilingual Style Guidelines

| Language     | Tone Style                | Key Characteristics                             |
| :----------- | :------------------------ | :---------------------------------------------- |
| **Korean**   | 친절하고 지적인 대화체    | 존댓말 사용, 신망 있는 전문가의 정중한 어조     |
| **English**  | Professional & Insightful | Analytical yet warm, clear and direct sentences |
| **Japanese** | 丁寧で共感的な文体        | です・ます調, 読者の心に寄り添う繊細な表現      |

## Future Blog Topics

### 1. Adlerian Psychology (Completed/Ongoing)

- [x] 지금처럼 살아도 괜찮을까?
- [x] 게으른 완벽주의자를 위한 아들러 심리학의 정수 5가지
- [x] 주어가 관계의 거리를 결정한다
- [x] 삶은 어려움을 극복하는 게임이다
- [x] 인생의 의미는 주변에 의해 정해지지 않는다
- [x] 인간은 자기 인생을 그리는 화가다
- [x] 지금의 결정 안에 미래가 있다
- [x] 행복은 삶을 책임질 때 비로소 모습을 보인다
- [x] 라이프스타일의 3가지 요소
- [x] 비교의 그늘 아래 자기 가치는 가려질 뿐이다
- [x] 목표를 이루었을 때의 모습을 구체적으로 그려라
- [x] 모두에게 중요한 존재일 필요 없다

### 2. Relationship & Courage (Next Focus)

- [x] 마침내 나를 지킬 용기를 내다
- [x] 곁에 둘 사람을 선택하는 것은 곧 미래를 선택하는 것이다
- [x] 인간관계는 자신이 뿜어낸 에너지의 결과물이다
- [x] 모든 고민은 인간관계에서 비롯된다
- [x] 무심코 내뱉는 말이 인연을 바꾼다
- [x] 인생의 목적이 분명한 자는 자기를 채워줄 사람을 만난다
- [x] 자기를 지키는 것이야말로 가장 고귀한 용기다
- [x] 타인을 바꾸려는 순간 내면의 평화는 깨진다
- [x] 내면에 존재하는 것은 사실이 아니라 그에 대한 해석뿐이다
- [x] 나에게 상처를 줄 수 있는 건 오직 나 자신뿐
- [x] 신뢰는 배신의 가능성을 감내하는 용기에서 피어난다
- [x] 팔로워 수는 신뢰의 증거가 아니다
- [x] ‘가로의 관계’로 용기를 부여하라
- [x] 모두에게 중요한 존재일 필요 없다

4장. 누구와 함께하느냐에 따라 인생의 깊이가 달라진다
[x] 마침내 나를 지킬 용기를 내다
[x] 곁에. 둘 사람을 선택하는 것은 곧 미래를 선택하는 것이다

- [x] 인간관계는 자신이 뿜어낸 에너지의 결과물이다
- [x] 모든 고민은 인간관계에서 비롯된다
- [x] 무심코 내뱉는 말이 인연을 바꾼다
- [x] 인생의 목적이 분명한 자는 자기를 채워줄 사람을 만난다
- [x] 자기를 지키는 것이야말로 가장 고귀한 용기다
- [x] 타인을 바꾸려는 순간 내면의 평화는 깨진다
- [x] 내면에 존재하는 것은 사실이 아니라 그에 대한 해석뿐이다
- [x] 나에게 상처를 줄 수 있는 건 오직 나 자신뿐
- [x] 신뢰는 배신의 가능성을 감내하는 용기에서 피어난다
- [x] 팔로워 수는 신뢰의 증거가 아니다
- [x] ‘가로의 관계’로 용기를 부여하라

5장. 세상은 선택한 만큼만 내 것이 된다
‘삶’이라는 그릇에 무엇을 담을 것인가
위대함을 꿈꾸는 자보다 자신을 믿는 자가 더 강하다

- [x] 미래를 담보로 과거에 붙잡히지 마라
- [x] 몸과 마음은 삶을 지탱하는 두 다리다
- [x] 가치 있는 소비로 삶에 의미를 더하다
- [x] 집은 행복을 위해 가꿀 수 있는 삶의 무대다

6장. 삶은 말보다 행동의 힘으로 바뀐다
행복에 필요한 자격은 없다
진정한 행복은 이제 시작일 뿐

- [x] 그 누구도 아닌 나에게 집중해야 하는 이유
- [x] ‘대상’이 아니라 ‘사람’에 주목하라
- [x] 누구에게나 행복을 선택할 힘이 있다

- [x] 인생의 방향키를 남에게 주지 마라
      ———————————————
      3장 외상 후 성장이란 무엇인가? [x]
      4장 유동적 요인 [x]
      5장 트라우마의 세대 간 유전 [x]
      6장 집단 트라우마에서 집단 성장으로 [x]

2부 단계를 지나 나아가기
7장 인식의 단계: 전적인 수용
8장 각성의 단계: 안전과 보호
9장 형성의 단계: 새로운 이야기
10장 존재의 단계: 통합
11장 전환의 단계: 지혜와 성장
12장 높은 곳에 머무르기

—————————————
신화와 정신분석

I부 신화 해석을 위한 정신분석의 기초

[x] 1. 신화 속에서 역동하는 힘들의 의미
[x] 2. 신화를 해석하는 정신분석의 관점들
[x] 3. 신화를 창조한 무의식의 유형들
[x] 4. 원시 인류의 신화적 사고
[x] 5. 정신분석의 눈으로 읽는 영웅신화

II부 신화와 정신분석
1장 수메르 신화

1. 길가메시 오만한 자가 현자로 변모하는 과정 [x]

2장 한국 신화

1. 창세신화 태초 인류의 두려움과 소망 [x]
2. 환인, 환웅, 웅녀, 단군 한민족 최초의 이상화 대상들 [x]
3. 주몽 어머니 애착으로 고뇌한 건국 영웅 [x]
4. 바리데기 버림받은 영혼이 치유자로 변환되는 과정 [x]

3장 중국 신화

1. 반고 천지 사이에 우뚝 선 중국 민족의 자아도취감 [x]
2. 예 끝내 행복해지지 못한 우울 자리 영웅의 인생 [x]
3. 순 거짓자기가 참자기로 바뀌기 위한 조건 [x]
   4장 일본 신화
4. 이자나기와 이자나미 갈등과 대립에서 균형과 안정으로 [x]
5. 스사노오 반사회적 성격이 창조적 에너지로 바뀌는 과정 [x]
6. 오오쿠니누시 민중의 좌절된 욕망을 보충해주는 이상화 대상

5장 이집트 신화

1. 오시리스, 이시스, 호루스 죽음에서 부활에 이르는 길 [x]

6장 그리스 신화

1. 그리스 창세 신화 지배자와 도전자의 끊임없는 투쟁 [x]
2. 오이디푸스 억압된 인간 욕망의 진실 [x]
3. 페르세우스 어머니의 영향에서 벗어나 주체가 되기 위한 조건 [x]
4. 테세우스 미궁 탈출과 반복되는 파국의 정체 [x]
5. 헤라클레스 박탈당한 모성성이 만들어낸 고통과 광기 [x]
6. 에로스와 프시케 감각적 쾌락과 정신적 기쁨이 공존하는 삶의 환희 [x]
7. 나르키소스와 에코 자기애 환상의 굴레에 갇힌 자의 비극 [x]

7장 북유럽 신화

1. 발드르 빛의 신이 죽었다가 살아나야 하는 이유 [x]
2. 오딘과 토르 유한한 세계를 살아가는 의지와 힘 [x]

III부 신화와 민족무의식

1. [x] 원시 인류의 정신성과 ‘왕 살해’ 모티프
2. [x] 창세신화와 동서양 세계관의 차이
3. [x] 영웅신화와 민족무의식의 콤플렉스들

IV부 인지 심리학과 메타인지

1.  아들러 심리학과 메타인지: 나를 객관적으로 바라보는 힘 [x]
2.  메타인지와 수업: 학습과 성장의 핵심 [x]
3.  메타인지를 높이는 5가지 수업 전략 [x]
4.  메타인지를 호출하는 7가지 수업 활동 [x]
5.  메타인지 중심 하부르타 모델 [x]

——————————————————
브레인 에너지 - 크리스토퍼 팔머

1부 모든 정신질환은 연결되어 있다
1장 방법을 바꾸려면 정신 건강의 실태부터 알아야 한다 [x]
정신질환은 꾸준히 확산되고 있다 23 | 정신 건강 문제가 만드는 또 다른 문제들 32 | 지금의 치료법은 괜찮은가? 35
2장 정신질환을 일으키는 원인은 무엇인가? [x]
우울증의 진짜 원인은 무엇일까 47 | 징후와 증상에 따른 치료법의 차이 58 | 모든 질환을 타이레놀로 치료하는 현실 63
3장 정신질환에는 반드시 공통경로가 있다 [x]
같지만 다른 하나의 정신질환 78 | 깊이 들여다보면 보이는 것 84 | 정신질환의 무수한 교집합 88
4장 신체질환도 정신질환과 연결되어 있을까? [x]
대사장애 98 | 신경학적 장애 103 | 등잔 밑이 어둡다 108

2부 밝혀진 연결고리, 뇌 에너지 이론
5장 정신질환은 대사 문제에서 온다 [x]
대사의 잔물결 효과 118 | 대사란 무엇인가 122 | 대사와 에너지 불균형 124 | 대사는 교통 흐름과 같다 129
6장 심적 상태와 정신질환 [x]
스트레스와 대사의 관계 134 | 스트레스가 신체세포에 미치는 영향 140 | 정신질환의 새로운 정의 144 | 세 대의 자동차 147 | 통증이 알려주는 것들 150 | 정신질환의 재정의 154 | 정신질환 증상이 발생하는 세 가지 상황 158 | 정신질환이라는 퍼즐을 푸는 뇌 에너지 이론 167
7장 희망의 공통경로, 미토콘드리아 [x]
미토콘드리아의 기원 171 | 현대의 미토콘드리아 174 | 세포를 켜고 끄는 미토콘드리아 179 | 세포 속 일꾼 미토콘드리아 194
8장 미토콘드리아와 뇌 에너지 불균형 [x]
미토콘드리아 기능부전과 정신 건강 200 | 미토콘드리아 기능부전이란 204 | 미토콘드리아 기능 이상과 정신질환 211 | 현실 속 뇌 에너지 이론 221 | 미토콘드리아와 정신질환을 이어주는 증거 227 | 진단명, 대사성 뇌 기능부전 235 | 정신장애는 뇌의 대사장애다 237

3부 뇌 에너지 이론이 가져올 혁명
9장 문제 원인과 대처 방법 찾기 [x]
증상과 장애가 개인별로 다른 이유 245 | 치료와 성공 사례 249
10장 정신질환은 가족력의 영향을 얼마나 받을까? [x]
유전 252 | 후생유전 257 | 유전과 후생유전으로 알 수 있는 정신질환의 원인과 치료법 263
11장 신경전달물질과 정신과 약의 효과 [x]
정신과 약, 대사, 그리고 미토콘드리아 273 | 요약 280 | 초조증에 시달리던 제인 281
12장 호르몬과 대사 조절자 [x]
코르티솔 287 | 인슐린 288 | 에스트로겐 294 | 갑상샘호르몬 298 | 요약 300 | 모든 것이 ‘갑상샘호르몬 때문’이었던 제임스 300
13장 우리의 생존을 돕는 염증 [x]
염증이 대사와 정신 건강에 미치는 영향 303 | 염증과 미토콘드리아의 관계 306 | 치료 과정에서 염증의 역할 309 | 요약 311
14장 수면과 일주기 리듬 [x] 313
수면, 빛, 일주기 리듬이 정신질환에 미치는 영향 320 | 수면, 빛 노출, 일주기 리듬 개선 322 | 요약 324 | 학교생활에 어려움을 겪던 칼렙 325
15장 [x]
비타민과 영양소 330 | 식품의 질 331 | 비만 336 | 미네소타 굶주림 실험 341 | 장내미생물의 역할 346 | 식이요법을 통한 정신 건강 완화 348 | 요약 358 | 70세에 시작한 치료로 정신 건강을 되찾은 밀드레드 359
16장 약물과 알코올이 정신질환을 일으키는 원리 361
알코올 364 | 마리화나 367 | 약물과 알코올의존치료 370 | 요약 371
17장 운동으로 모든 걸 해결할 수 없다 [x] 372
운동의 이로운 효과 379 | 요약 380
18장 분명한 삶의 목적이 갖는 힘 [x] 381
일상 속 스트레스가 건강에 미치는 영향 386 | 목적의식이 중요한 이유 388 | 삶의 목적에 초점을 맞춘 치료법과 효과 391 | 요약 400 | 필라테스로 정신질환을 극복한 사라 401
19장 뇌 에너지 이론으로 살펴보는 기존 치료법 [x] 403
전기경련요법과 경두개자기자극술 404 | 뇌수술과 전기자극치료 406 | 요약 407
20장 대사 치료 계획 접근법 [x]
21장 대사 건강과 정신 건강을 위한 새로운 시작 [x]
————————————————————————
[x] 마광수 인생조언: 허무 속에서 성실하게 사는 법
[x] 테스토스테론 높이는 방법(테토남, 테토녀 되는 방법)
[x] 월별 추천 해외여행지
[x] 멘탈관리에 도움이 되는 글귀
[x] 고졸출신이 OpenAI 에 입사한 과정

## Writing Checklist (Audit)

- [x] All 11 recent Korean posts exceed 3000 characters.
- [x] All English translations completed and linked.
- [x] All Japanese translations completed and linked.
- [x] Frontmatter descriptions optimized for Astro (< 200 chars).
