    ...
    <div className="not-prose my-6 sm:my-10 p-4 sm:p-8 bg-white border border-slate-200 rounded-3xl shadow-xl max-w-2xl mx-auto space-y-5 sm:space-y-8">
            ...
            <div className="grid grid-cols-1 gap-2">
              {([q.optionA, q.optionB] as { text: string; value: DimValue }[]).map((opt) => (
                <button
                  ...
                  className={`py-3 px-4 text-sm rounded-xl border transition-all text-left ${answers[q.id] === opt.value ? "bg-amber-500 border-amber-500 text-white font-bold shadow-md" : "bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100"}`}
                >
                  {opt.text}
                </button>
              ))}
      ...
      <div className="flex justify-center pt-4">
        <button
          ...
          className={`px-5 sm:px-10 py-3 rounded-2xl font-bold text-base transition-all ${isComplete ? "bg-amber-500 text-white hover:bg-amber-600 shadow-lg" : "bg-slate-200 text-slate-400 cursor-not-allowed"}`}
        >
