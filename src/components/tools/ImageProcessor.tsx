...
export default function ImageProcessor({ locale = "ko", initialMode = "optimize" }: { locale?: Locale; initialMode?: Mode }) {
  ...
  return <GameContainer title={t[0]} subtitle={t[1]} onReset={reset}>
    <div className="space-y-5">
      ...
      {!source ? <button type="button" className="w-full rounded-xl border-2 border-dashed border-border bg-muted/30 px-4 py-6 sm:py-12" onClick={() => inputRef.current?.click()}><span className="block font-semibold">{t[4]}</span><span className="mt-2 block text-sm text-muted-foreground">{t[11]}</span></button> : <>
