    ...
    categories: [
      {
        title: "💰 임금 & 최저임금",
        items: [
          { q: "2026년 최저임금은 얼마인가요?", a: "시급 10,320원입니다. 월 환산액(209시간 기준)은 2,156,880원입니다." },
...
const LaborLawQuickCheck: React.FC<{ locale?: 'ko' | 'en' }> = ({ locale = 'ko' }) => {
    ...
    return (
        <div className="not-prose my-7 sm:my-12 p-4 sm:p-8 bg-slate-900 text-white rounded-3xl shadow-2xl border border-slate-800">
            <h3 className="text-2xl font-black text-center mb-5 sm:mb-8 bg-gradient-to-r from-blue-400 to-emerald-400 bg-clip-text text-transparent">
                {t.title}
            </h3>
