...
$$
\begin{aligned}
Y_t &= T_t \times S_t \times C_t \times I_t (\text{승법 모형}) \\
Y_t &= T_t + S_t + C_t + I_t (\text{가법 모형})
\end{aligned}
$$
...
**차분(Differencing)**:
$$
\Delta Y_t = Y_t - Y_{t-1} \quad \text{(1차 차분)}
$$
...
$$
\begin{aligned}
MAE &= \sum |Y_t - \hat{Y}_t| / n \quad \text{(평균절대오차)} \\
MSE &= \sum (Y_t - \hat{Y}_t)^2 / n \quad \text{(평균제곱오차)} \\
RMSE &= \sqrt{MSE} \quad \text{(평균제곱근오차)} \\
MAPE &= \sum |Y_t - \hat{Y}_t| / Y_t / n \times 100\% \quad \text{(평균절대백분율오차)}
\end{aligned}
$$
