---
track: academy
series: macroeconomics
chapter: 6
title: "거시경제학 — 투자함수론: 미래를 위한 선택"
description: "현재가치법, 한계효율설, 그리고 토빈의 q 이론으로 기업의 투자 결정 원리를 분석합니다. 가속도 원리와 신축적 가속도 모형을 다룹니다."
pubDate: 2026-03-01T00:00:00.000Z
category: Economics
tags:
  - Education
  - economics
  - 정병렬경제학
locale: ko
author: Oiyo
---

import {
  LectureTable,
} from "@/features/education-common/components/LectureVisuals";
import FormulaBox from "@/components/mdx/FormulaBox.astro";
import Callout from "@/components/mdx/Callout.astro";

# 거시경제학 — 투자함수론: 미래를 위한 선택

투자는 소비보다 규모는 작지만, 경기 변동을 일으키는 가장 가변적이고 핵심적인 요소이다. 기업이 어떤 기준으로 기계 설비를 늘리고 공장을 짓는지, 투자의 경제적 결정 요인들을 정밀하게 분석한다.

---

## 1. 투자 결정의 전통적 이론

### (1) 현재가치법 (NPV: Net Present Value)

미래에 발생할 수익을 현재 가치로 할인하여 투자비용과 비교한다.

<FormulaBox>
  $NPV = \sum \frac{R_t}{(1+i)^t} - I > 0$ 이면 투자 실행.
</FormulaBox>

### (2) 자본의 한계효율설 (MEI: Marginal Efficiency of Investment)

- 투자의 한계효율($r$)이 시장 이자율($i$)보다 높으면 투자를 실행한다.
- **케인즈의 관점**: 투자는 이자율뿐만 아니라 기업가의 '야성적 충동(Animal Spirits)'에 의해 크게 좌우된다고 보았다.

---

## 2. 가속도 원리 (Acceleration Principle)

소비재 수요의 변화가 자본재(투자) 수요에 더 큰 폭의 변화를 가져온다는 원리이다.

- **원리**: 생산량의 **증가분**에 비례하여 투자가 결정된다.
- **한계**: 설비 가동률이 낮을 때는 작동하지 않으며, 실제 현실보다 과장된 측면이 있다 (신축적 가속도 모형이 보완).

---

## 3. 토빈의 q 이론 (Tobin's q Theory)

금융 시장(주식 시장)의 정보를 투자의 지표로 활용하는 현대적 이론이다.

<FormulaBox>
  $q = \frac{\text{주식시장에서 평가된 기업의 가치}}{\text{기업 자산의 실물 대체 비용}}$
</FormulaBox>

- **$q > 1$**: 기업 실물 가치보다 주식 가치가 높음 $\rightarrow$ **투자 확대**.
- **$q < 1$**: 투자를 하는 것보다 기업을 사는 것이 쌈 $\rightarrow$ **투자 축소**.
- <mark>**시사점**</mark>: 주가 지수가 실물 경제의 선행 지표가 되는 이론적
  근거를 제공한다.

---

## 4. 자본의 사용자 비용 (User Cost of Capital)

신고전학파의 투자 이론으로, 자본 1단위를 유지하는 데 드는 비용과 한계생산성을 비교한다.

- **사용자 비용**: 이자 비용 + 감가상각비 - 자산 가격 상승률.
- **최적 투자**: 자본의 한계생산(MPK) = 사용자 비용 이 되는 지점에서 결정.

---

## 5. 마무리: 투자는 내일의 성장판입니다

투자는 단기적으로는 총수요의 변동을 가져오지만, 장기적으로는 생산 능력을 확충하여 성장의 기반이 된다. 이자율에 민감한 투자의 성격은 금융 정책이 실물 경제로 전달되는 가장 중요한 통로가 된다.

---

## 📖 참고문헌

- **[거시경제학] - 정병렬**: 투자 이론의 수리적 도출.
- **[기업 투자 이론] - 제임스 토빈**: q 이론의 정립.

---

수고하셨습니다. 다음 시간에는 가계와 기업의 행동이 모여 형성되는 **IS-LM 모형의 심층 분석과 정책 유효성**에 대해 학습하겠습니다.

## 확인문제

자본 한계효율이 12%, 대출 금리 8%면 투자는 늘어난다. 금리가 13%가 되면 같은 프로젝트는 기각된다. 이자율 한 점이 투자함수의 위치다.

## 참고 자료

- N. Gregory Mankiw, *Macroeconomics*
- MIT OpenCourseWare, [14.02 Principles of Macroeconomics](https://ocw.mit.edu/courses/14-02-principles-of-macroeconomics-spring-2023/)
- 한국은행, [국민계정 해설](https://www.bok.or.kr/portal/main/contents.do?menuNo=200554)
