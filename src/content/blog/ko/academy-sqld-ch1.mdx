...
track: academy
locale: ko
title: "SQLD 자격증 — SQL 기초와 SELECT 완전 정복"
...
> WHERE 절에서는 집계함수를 사용할 수 없습니다. 집계함수는 GROUP BY 이후에 적용되기 때문에 WHERE 처리 시점에는 값이 결정되지 않습니다. 올바른 방법은 서브쿼리를 사용하는 일입니다: `WHERE salary > (SELECT AVG(salary) FROM employees)`.
