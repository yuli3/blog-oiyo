...
상자의 아래 끝이 Q1(25분위수), 가운데 선이 중앙값(Q2), 위 끝이 Q3(75분위수)이고, 상자 위아래로 뻗은 수염 끝이 최솟값·최댓값이다. 수염 밖에 따로 찍힌 점은 이상치(IQR×1.5 초과)다.
...
$$
\begin{aligned}
\text{공식}: r &= \sum(x_{i} - x̄)(y_{i} - ȳ) / \sqrt[\sum(x_{i} - x̄)^2 \times \sum(y_{i} - ȳ)^2] \\
&\text{범위}: -1 \leq r \leq 1 \\
&\text{해석}: \\
r &= 1.0: \text{완전 양의 선형 관계} \\
r &= 0.7: \text{강한 양의 선형 관계} \\
r &= 0.3: \text{약한 양의 선형 관계} \\
r &= 0.0: \text{선형 관계 없음} \\
r &= -0.3: \text{약한 음의 선형 관계} \\
r &= -0.7: \text{강한 음의 선형 관계} \\
r &= -1.0: \text{완전 음의 선형 관계} \\
&\text{주의}: \text{피어슨} r\text{은 비선형 관계를 포착하지 못함} \\
&\text{두 변수가 비선형 관계이더라도} r \approx 0\text{일 수 있음}
\end{aligned}
$$
...
4. 다중공선성
  - 독립변수들 간 높은 상관관계 (|r| > 0.8)는 회귀분석에서 계수 추정 불안정 야기
