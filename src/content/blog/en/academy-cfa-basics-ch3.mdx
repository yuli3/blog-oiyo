...
$$
\begin{aligned}
&\text{Bond Price Formula}: \\
P &= \sum [\frac{C}{1 + r}^t] + [\frac{FV}{1 + r}^n] \\
&\text{Where}: \\
P &= \text{current bond price} \\
C &= \text{coupon payment per period} (= \text{Face Value} \times \frac{\text{coupon rate}}{\text{payments per year}}) \\
r &= \text{periodic discount rate} (= \frac{YTM}{\text{payments per year}}) \\
n &= \text{total number of periods} \\
FV &= \text{face value} (\text{par repaid at maturity}) \\
t &= \text{timing of each cash flow} (1, 2, ..., n)
\end{aligned}
$$
...
$$
\begin{aligned}
\text{Semi-annual coupon} &= \$1,000 \times 6\% / 2 = \$30 \\
\text{Semi-annual discount rate} r &= 8\% / 2 = 4\% \\
\text{Total periods} n &= 5 \times 2 = 10 \\
P &= \frac{30}{1.04} + \frac{30}{1.04^2} + ... + \frac{30}{1.04}¹⁰ + \frac{1,000}{1.04}¹⁰ \\
&= 30 \times [1 - (1.04)⁻¹⁰] / 0.04 + \frac{1,000}{1.04}¹⁰ \\
&\approx 243.32 + 675.56 \\
&\approx \$918.88
\end{aligned}
$$
...
- P = Σ [C / (1 + YTM/2)^t] + [FV / (1 + YTM/2)^n] → Compute YTM using the BA II Plus calculator
...
\text{Example}: \$60 \text{coupon} / \$918 \text{price} &= 6.54\%
\end{aligned}
$$
...
**Example**: 3-year bond, 8% coupon, YTM 8% → D_Mac ≈ 2.78 years
...
- D_Mod = D_Mac / (1 + YTM/m) where m = coupon payments per year

**Price change approximation**

- %ΔP ≈ -D_Mod × ΔYTM
- **Example**: D_Mod = 4.5, YTM rises by 0.5% (+0.005)
- %ΔP ≈ -4.5 × 0.005 = -2.25% → Bond price expected to fall ~2.25%
...
**Improved price change (duration + convexity)** — %ΔP ≈ -D_Mod × ΔYTM + (1/2) × Convexity × (ΔYTM)²

**Convexity effect**

- When yields rise: actual price decline is smaller than duration estimate
- When yields fall: actual price increase is larger than duration estimate → Greater convexity is always favorable to investors
...
$$
\text{Spread} = \text{compensation for credit risk} + \text{liquidity risk}
$$
