...
$$
\begin{aligned}
\text{Capital gain} &= \text{Sale price} - \text{Cost basis} - \text{Transaction fees} \\
\text{Taxable income} &= \text{Total annual capital gains} - \text{Exemption threshold} \\
\text{Tax owed} &= \text{Taxable income} \times \text{applicable tax rate}
\end{aligned}
$$
...
- **Airdrops**: Treated as income at market value when received; subsequent sale may trigger an additional capital gain
