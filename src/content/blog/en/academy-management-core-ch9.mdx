...
track: academy
locale: en
title: 'Management Science — Making Decisions with Data and Mathematics'
category: "Business"
...
- **Launch**
  - Boom (probability 0.6) → +\$500k
  - Bust (probability 0.4) → −\$150k
- **No launch**
  - Certain → \$0
...
### (2) Basic LP Structure

$$
\begin{aligned}
\text{Maximize } & Z = 5X + 4Y && \text{(objective function: profit)} \\
\text{subject to } & 6X + 4Y \le 24 && \text{(resource A usage limit)} \\
& X + 2Y \le 6 && \text{(resource B usage limit)} \\
& X,\ Y \ge 0 && \text{(non-negativity constraints)}
\end{aligned}
$$
...
### (1) EOQ (Economic Order Quantity) Revisited
...
**Reorder Point (ROP)**:
$$
ROP = \text{Average demand during lead time} + \text{Safety stock}
$$
...
$$
\begin{aligned}
\text{Average waiting time} &= \rho / [\mu(1 − \rho)] \\
\text{Average customers in system} &= \frac{\rho}{1 − \rho}
\end{aligned}
$$
...
## 8. Limitations of Management Science
