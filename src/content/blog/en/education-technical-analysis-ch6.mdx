...
61.8% retracement → deep pullback (weakening trend) → A break below 61.8% signals a possible trend reversal
...
$$
\begin{aligned}
&A (\text{swing low}) \to B (\text{swing high}) \to C (\text{retracement low}) \\
\text{Target} &= C + (A \to B \text{price range} \times 1.618) \\
&\text{Example}: \\
A &= \$10.00, B = \$15.00, C = \$12.50 \\
A \to B \text{range} &= \$5.00 \\
\text{Target} &= \$12.50 + (\$5.00 \times 1.618) = \$20.59
\end{aligned}
$$
...
$$
5 \text{impulse waves} (\text{upward}) + 3 \text{corrective waves} (\text{downward}) = \text{one complete cycle}
$$
...
- Alternation principle: Waves 2 and 4 tend to take different forms (one simple, one complex)
