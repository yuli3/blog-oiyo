...
track: academy
locale: en
title: 'Correlation and Regression Analysis — Analyzing Relationships Between Variables'
category: "Statistics"
...
$$
\begin{aligned}
r &= \sum[(x_{i} − x̄)(y_{i} − ȳ)] / \sqrt[\sum(x_{i}−x̄)^2 \times \sum(y_{i}−ȳ)^2] \\
&\text{Range}: −1 \leq r \leq 1
\end{aligned}
$$
...
$$
\begin{aligned}
\beta_{1} &= \sum(x_{i}−x̄)(y_{i}−ȳ) / \sum(x_{i}−x̄)^2 = r \times (\frac{\text{sy}}{\text{sx}}) \\
\beta_{0} &= ȳ − \beta_{1}x̄
\end{aligned}
$$
...
$$
Y = \beta_{0} + \beta_{1}X_{1} + \beta_{2}X_{2} + ... + \betaₖXₖ + ε
$$
