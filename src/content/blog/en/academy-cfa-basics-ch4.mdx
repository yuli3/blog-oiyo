...
$$
\begin{aligned}
&\text{Basic Futures Price Formula} (\text{no carrying costs}, \text{financial asset}): \\
F_{0} &= S_{0} \times (1 + r)ᵀ \\
&\text{Where}: \\
F_{0} &= \text{theoretical futures price} \\
S_{0} &= \text{current spot price} \\
r &= \text{risk-free rate} (\text{annualized}) \\
T &= \text{time to maturity} (\text{years}) \\
&\text{For dividend-paying assets} (\text{stocks}, \text{indices}): \\
F_{0} &= (S_{0} - PV(D)) \times (1 + r)ᵀ \\
&\text{or with continuous dividend yield} δ: \\
F_{0} &= S_{0} \times e^(r - δ)T \\
&\text{For commodities} (\text{storage cost} c, \text{convenience yield} y): \\
F_{0} &= S_{0} \times e^(r + c - y)T
\end{aligned}
$$
...
$$
\begin{aligned}
&\text{Put-Call Parity} (\text{European options}): \\
C + PV(X) &= P + S_{0} \\
&\text{Alternatively}: \\
C - P &= S_{0} - PV(X) \\
&\text{Where}: \\
C &= \text{call premium} \\
P &= \text{put premium} \\
S_{0} &= \text{current underlying price} \\
PV(X) &= \text{present value of strike} = \frac{X}{1 + r}^T \\
&\text{Synthetic Positions}: \\
\text{Synthetic call} &= \text{long put} + \text{long underlying} − \text{long bond} \\
\text{Synthetic put} &= \text{long call} − \text{long underlying} + \text{long bond} \\
\text{Synthetic stock} &= \text{long call} + \text{long bond} − \text{long put}
\end{aligned}
$$
...
- J-Curve effect: negative returns early (fees, underperforming investments) → performance recovers as portfolio matures and exits occur
