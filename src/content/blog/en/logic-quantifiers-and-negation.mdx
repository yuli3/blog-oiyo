...
$$
\begin{aligned}
\neg (All A are B) &= Some A are not B (there is an A that is not B) \\
\neg (Some A are B) &= No A are B (there is no A that is B) \\
\neg (If A, then B) &= There is a case where A but not B
\end{aligned}
$$
