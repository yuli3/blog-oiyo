...
track: academy
locale: en
title: "Regression Analysis and Time Series Forecasting"
category: "Exam"
...
tags:
  ...
  - ARIMA
  - linear regression
description: "Linear regression, Ridge/Lasso regularization, time series decomposition, ARIMA — predicting numerical outcomes."
pubDate: '2026-05-31'
---
...
&\beta_{0}: \text{intercept}, \beta_{1}: \text{slope} (\text{coefficient}), ε: \text{error}
\end{aligned}
$$
...
$$
Y = \beta_{0} + \beta_{1}X_{1} + \beta_{2}X_{2} + ... + \beta_{n}X_{n} + ε
$$
...
$$
\text{Cost} = MSE + \lambda\sum\beta_{i}^2
$$
...
$$
\text{Cost} = MSE + \lambda\sum|\beta_{i}|
$$
...
$$
Y_{t} = c + φ_{1}Y_{t-1} + ... + φₚY_{t-}ₚ + ε_{t}
$$
