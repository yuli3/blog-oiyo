...
track: academy
locale: en
title: 'Tax Intro — Corporate Income Tax Basics'
category: "Tax"
...
$$
\begin{aligned}
&\text{Tax Reconciliation} (\text{Schedule} M-1): \\
&\text{Book net income} \\
&+ \text{Book income not included in taxable income} (\text{add-backs}) \\
&+ \text{Non-deductible book expenses} \\
&− \text{Tax deductions not in book income} \\
&− \text{Book income items exempt from tax} \\
&= \text{Taxable income}
\end{aligned}
$$
...
<HighlightBox title="Quarterly estimated payments due">
