...
track: academy
locale: en
title: 'Time Series Analysis — Forecasting the Future from Historical Data'
category: "Statistics"
...
$$
\hat{Y}_{t+1} = \alpha \times Y_t + (1−\alpha) \times \hat{Y}_t
$$
...
**Differencing**:
$$
\Delta Y_t = Y_t - Y_{t-1} \quad \text{(first-order differencing)}
$$
...
$$
\begin{aligned}
MAE &= \sum |Y_t - \hat{Y}_t| / n \quad \text{(Mean Absolute Error)} \\
MSE &= \sum (Y_t - \hat{Y}_t)^2 / n \quad \text{(Mean Squared Error)} \\
RMSE &= \sqrt{MSE} \quad \text{(Root Mean Squared Error)} \\
MAPE &= \sum |Y_t - \hat{Y}_t| / Y_t / n \times 100\% \quad \text{(Mean Absolute Percentage Error)}
\end{aligned}
$$
