...
$$
\begin{aligned}
&\text{Gross Estate} (\text{fair market value of all assets}) \\
&+ \text{Taxable Gifts Made During Life} (\text{past} 3 \text{years for certain transfers}) \\
&− \text{Allowable Deductions}: \\
&\text{Marital deduction} (\text{unlimited for citizen spouses}) \\
&\text{Charitable deduction} (\text{unlimited for qualifying charities}) \\
&\text{Debts}, \text{mortgages}, \text{funeral expenses} \\
&= \text{Taxable Estate} \\
&− \text{Unified Credit} (\$13.61M \text{exemption equivalent}, 2024) \\
&= \text{Amount Subject to Tax} \\
&\times \text{Applicable Rate} (\text{up to} 40\%) \\
&= \text{Tentative Estate Tax} \\
&− \text{Credits} (\text{state death tax credit}, \text{etc}.) \\
&= \text{Net Federal Estate Tax Due}
\end{aligned}
$$
