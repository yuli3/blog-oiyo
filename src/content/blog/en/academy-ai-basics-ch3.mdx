...
$$
\text{Output} = \text{Activation}(\sum(\text{weight} \times \text{input}) + \text{bias})
$$
...
$$
\text{weight} \leftarrow \text{weight} - \text{learning\_rate} \times \partial \text{loss}/\partial \text{weight}
$$
