...
\begin{aligned}
&X ~ B(n, p) \\
P(X &= k) = C(n,k) \times pᵏ \times (1−p)ⁿ⁻ᵏ \\
\text{Expected value} E(X) &= \text{np} \\
\text{Variance Var}(X) &= \text{np}(1−p)
\end{aligned}
$$
...
\begin{aligned}
&X ~ N(\mu, \sigma^2) \\
f(x) &= (1/\sqrt(2\pi\sigma^2)) \times \text{exp}(−(x−\mu)\frac{^2}{2}\sigma^2)
\end{aligned}
$$
...
\begin{aligned}
Z &= (X − \mu) / \sigma \\
&Z ~ N(0, 1) \quad \text{(standard normal distribution)}
\end{aligned}
$$
...
\begin{aligned}
&X ~ \text{Exp}(\lambda) \\
E(X) &= 1/\lambda \quad \text{(mean waiting time)} \\
\text{Var}(X) &= 1/\lambda^2 \\
\text{Memoryless Property}: P(X > s+t | X > s) &= P(X > t)
\end{aligned}
$$
