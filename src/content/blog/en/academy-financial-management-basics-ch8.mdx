...
$$
\begin{aligned}
\text{Long}: P\ & L \&= \text{Settlement Price} − \text{Futures Price} \\
\text{Short}: P\ & L \&= \text{Futures Price} − \text{Settlement Price}
\end{aligned}
$$
...
$$
\text{Call Price} − \text{Put Price} = S_0 − \frac{K}{1 + r}^T
$$
...
- B pays A the fixed rate → Each party exploits its comparative advantage
