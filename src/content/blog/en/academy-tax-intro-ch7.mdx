...
$$
\begin{aligned}
&\text{Gross Estate} (\text{FMV of all included assets}) \\
&− \text{Deductions}: \\
&\text{Marital deduction} (\text{unlimited} — \text{assets to US citizen spouse}) \\
&\text{Charitable deduction} (\text{unlimited} — \text{assets to qualifying charities}) \\
&\text{Debts}, \text{funeral expenses}, \text{estate administration fees} \\
&\text{Mortgages and other liabilities} \\
&= \text{Taxable Estate} \\
&+ \text{Adjusted Taxable Gifts} (\text{post}-1976 \text{taxable gifts not in gross estate}) \\
&= \text{Estate Tax Base} \\
&\times \text{Rate Schedule} \\
&= \text{Tentative Tax} \\
&− \text{Unified Credit} (\text{equals tax on the basic exclusion amount}) \\
&= \text{Estate Tax Owed}
\end{aligned}
$$
...
**When Lifetime Exemption is exhausted** — Gift tax rate = 40% flat on taxable gifts
...
**10 years of education payments to grandchildren's colleges under §2503(e)** — $30,000/year per grandchild × 4 × 10 years = $1,200,000 excluded (no gift tax)
