...
track: academy
locale: en
title: 'Descriptive Statistics — Summarizing Data with Mean, Variance, and Standard Deviation'
category: "Statistics"
...
$$
\text{Arithmetic Mean} = \frac{(\text{Sum of all values})}{\text{Number of data points}}
$$
...
$$
\begin{aligned}
\text{Population variance} \sigma^2 &= \sum(X_{i} − \mu)\frac{^2}{N} \\
\text{Sample variance} s^2 &= \sum(x_{i} − x̄)\frac{^2}{n−1}
\end{aligned}
$$
...
s &= \sqrt{\text{Sample variance}} \quad (\text{sample})
\end{aligned}
$$
