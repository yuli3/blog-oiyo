...
$$
\begin{aligned}
\text{Beginning carrying amount} \times \text{effective interest rate} &= \text{interest income} \\
\text{Interest income} − \text{cash interest received} &= \text{amortization of discount} \\
\text{Ending carrying amount} &= \text{beginning} + \text{amortization} (\text{discount bonds})
\end{aligned}
$$
...
$$
\begin{aligned}
\text{Lease liability} &= \text{PV of remaining lease payments} (\text{at lessee's} \\
&\text{incremental borrowing rate if implicit rate unknown}) \\
\text{Right-of-use asset} &= \text{Lease liability} \\
&+ \text{Lease incentives paid to lessor} \\
&+ \text{Initial direct costs} \\
&+ \text{Prepaid lease payments} \\
&− \text{Lease incentives received}
\end{aligned}
$$
...
1. Compare book carrying value vs. tax basis for each asset/liability
2. Compute temporary differences
  - Taxable temporary difference → Deferred tax liability
  - Deductible temporary difference → Deferred tax asset
3. Apply enacted future tax rate (when difference reverses)
4. Assess valuation allowance for deferred tax assets (more-likely-than-not criterion)
...
&= \text{Ending inventory units} \times \text{Fixed overhead per unit} \\
&− \text{Beginning inventory units} \times \text{Fixed overhead per unit}
\end{aligned}
$$
...
- Number of performance obligations: 2 (software license + maintenance service)

[Calculations]

1. Standalone selling prices: Software $900,000; Maintenance $300,000
2. Allocation ratios: Software 75%, Maintenance 25%
3. Transaction price $1,000,000 allocated:
  - Software: $750,000
  - Maintenance: $250,000
