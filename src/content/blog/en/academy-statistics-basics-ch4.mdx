...
&Sample mean \bar{x} ~ N(\mu , \sigma ^2/n) \\
\text{Expected value}: E(\bar{x}) &= \mu \\
\text{Variance}: Var(\bar{x}) &= \sigma ^2/n \\
...
$$
\begin{aligned}
&\bar{x} ~ N(\mu , \sigma ^2/n) \quad \text{(when n is sufficiently large)} \\
\text{Standardization}: Z &= \frac{\bar{x} - \mu }{\frac{\sigma}{\sqrt{n}}}
\end{aligned}
$$
...
$$
\begin{aligned}
SE &= \frac{\sigma}{\sqrt{n}} \quad \text{(when population SD is known)} \\
SE &= \frac{s}{\sqrt{n}} \quad \text{(when estimating with sample SD)}
\end{aligned}
$$
...
$$
\begin{aligned}
\text{Sample proportion } \hat{p} &= \dfrac{\text{number of successes in sample}}{n} \\
\text{Expected value}: E(\hat{p}) &= p \\
\text{Variance}: Var(\hat{p}) &= p(1-p)/n \\
