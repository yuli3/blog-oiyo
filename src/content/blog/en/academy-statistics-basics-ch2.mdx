...
track: academy
locale: en
title: 'Probability Theory — Quantifying Uncertainty with Numbers'
category: "Statistics"
...
\begin{aligned}
P(A∪B) &= P(A) + P(B) − P(A∩B) \\
\text{If mutually exclusive}: P(A∪B) &= P(A) + P(B)
\end{aligned}
$$
...
$$
P(A \mid B) = P(A∩B) / P(B) (\text{provided} P(B) > 0)
$$
...
$$
\begin{aligned}
\text{Independence condition}: P(A∩B) &= P(A) \times P(B) \\
P(A \mid B) &= P(A)
\end{aligned}
$$
...
\text{Three events}: P(A \cap B \cap C) &= P(A) \times P(B \mid A) \times P(C \mid A \cap B)
\end{aligned}
$$
...
- **Specificity (true negative rate)**: $P(\text{Negative} \mid \text{Healthy}) = 0.95$
$$
P(\text{Positive } \mid \text{Healthy}) = 0.05
$$

$$
P(\text{Disease } \mid \text{Positive}) = \frac{0.99  \times  0.01}{0.99  \times  0.01 + 0.05  \times  0.99}
$$
≈ 16.7%
